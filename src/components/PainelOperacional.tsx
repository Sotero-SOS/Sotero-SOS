...
export default function PainelOperacional() {
    ...
    const carregar = async () => {
        ...
        const [aRes, mRes, moRes, vRes] = await Promise.all([
            supabase.from('atendimento').select('*').order('nr_atendimento', { ascending: false }),
    ...
    const dadosFiltrados = itens.filter(i => correspondeFiltro(i) && corresponderBusca(i));
    ...
    const concluirChamado = async (i: AtendimentoEnriquecido) => {
        ...
        const confirmacao = confirm(`Deseja marcar o atendimento #${i.nr_atendimento} como concluído?`);
        ...
        if (error) {
            ...
            alert(
                `Atendimento #${i.nr_atendimento} marcado como Fechado${marcadoAtrasado ? ' — Atrasado' : ''}`
            );
            await carregar();
        }
    };
        ...
        <div className="card">
            ...
            <div className="filtros">
                <div className="grupo-botoes">
                    <button className={filtro === 'aberto' ? 'ativa' : ''} onClick={() => setFiltro('aberto')}>
                        Abertos ({contagens.aberto})
                    </button>
                    <button className={filtro === 'fechado' ? 'ativa' : ''} onClick={() => setFiltro('fechado')}>
                        Fechados ({contagens.fechado})
                    </button>
                    <button className={filtro === 'atrasado' ? 'ativa pulsando' : ''} onClick={() => setFiltro('atrasado')}>
            ...
            {dadosFiltrados.length > 0 ? (
                <div className="cards-grid">
                    {dadosFiltrados.map((i) => {
                        ...
                        const excedeuPrevisto = (i.tempo_decorrido_ms ?? 0) > previstoMs && previstoMs > 0;
                            ...
                            <div key={i.nr_atendimento} className="card cartao-sos">
                                ...
                                >
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                        <strong style={{ fontSize: 16 }}>#{i.nr_atendimento}</strong>
                                        <strong
                                            className={`tempo-topo ${excedeuPrevisto ? 'aviso-topo' : ''}`}
                                            title={i.tempo_decorrido ? `Tempo decorrido: ${i.tempo_decorrido}` : 'Sem início'}
                    ...
                    <span className={`badge ${i.status === 'Fechado' ? 'ok' : i.atrasado ? 'erro' : 'proc'}`}>
                      {i.status ?? '—'}
                    </span>
                                        {i.atrasado && <span className="badge aviso">Atrasado</span>}
                                    </div>
                                </div>
                                ...
                                <div className="lin">
                                    ...
                                    <span className="val">
        {i.matricula_motorista} {i.motorista_nome ? `- ${i.motorista_nome}` : ''}
    </span>
                                </div>
                                ...
                                <div className="lin">
                                    ...
                                    <span className="val">
        {i.veiculo
            ? `${i.veiculo.cod_veiculo} ${i.veiculo.categoria ? `(${i.veiculo.categoria})` : ''}`
            : i.cod_veiculo
        }
    </span>
                                </div>
                                ...
                                <div className="lin">
                                    ...
                                    <span className="val">{i.motivo_descricao ?? i.cod_motivo}</span>
                                </div>
                                <div className="lin">
                                ...
                                <div className="lin">
                                    ...
                                    <span className="val">{formatarHora(i.chegada_na_garagem)}</span>
                                </div>
                                <div className="lin">
                                ...
                                <div className="acoes">
                                    <button className="secundario" onClick={() => handleEditar(i.nr_atendimento)}>
                                        Editar
                                    </button>
                                    {i.status !== 'Fechado' && (
                                        <button className="secundario" onClick={() => concluirChamado(i)}>
                                            Concluir
                                        </button>
                                    )}
                                </div>
                ...
                <div style={{ textAlign: 'center', padding: '24px 0' }}>
                    <p className="status">
                        {itens.length === 0 ? 'Nenhum chamado SOS encontrado.' : 'Nenhum chamado para os filtros aplicados.'}
                    </p>
                    <p className="status">
                        {itens.length === 0
