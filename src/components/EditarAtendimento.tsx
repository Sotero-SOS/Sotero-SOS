...
export default function EditarAtendimento({ id: selectedId = null, onClose }: Props) {
    ...
    const carregar = async () => {
        ...
        if (selectedId) {
            atendimentoQuery = atendimentoQuery.eq('nr_atendimento', selectedId).limit(1);
        } else {
    ...
    const atualizar = async (id: number) => {
        ...
        if (chg) {
            ...
            if (item) {
                ...
                const final = parseDataHora(item.data ?? null, updatePayload.final_sos ?? null);
            ...
            <div className="lista">
                {itensMostrados.map(a => {
                    ...
                    return (
                        <div key={a.nr_atendimento} className="linha" style={{ border: '1px solid #eee', padding: 12, borderRadius: 6, marginBottom: 12 }}>
                            <div className="col" style={{ flex: 1, minWidth: 260 }}>
                                ...
                                <div>Veículo: {a.veiculo ? `${a.veiculo.cod_veiculo}${a.veiculo.categoria ? ` (${a.veiculo.categoria})` : ''}` : show(a.cod_veiculo)}</div>
                                <div>Motivo: {a.motivo_descricao ?? a.cod_motivo ?? '—'}</div>
                                <div>Local: {show(a.local)}</div>
                                <div>Data: {show(a.data)}</div>
                                <div>Início SOS: {formatarHora(a.inicio_sos)}</div>
                                <div>Chegada na garagem: {formatarHora(a.chegada_na_garagem)}</div>
                                <div>Final SOS: {formatarHora(a.final_sos)}</div>
                                <div>Status: {show(a.status)}</div>
                                <div>Tempo previsto: {a.tempo_previsto_str ?? '—'}</div>
                                <div>
                                    Tempo decorrido:{' '}
                    ...
                    {a.tempo_decorrido ?? '—'}
                                ...
                                <div>Matrícula motorista: {show(a.matricula_motorista)}</div>
                            ...
                            <div className="col" style={{ flex: 1, minWidth: 240, display: 'flex', flexDirection: 'column', gap: 8 }}>
                                <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                    ...
                                    <input
                                        type="text"
                                        value={auxiliar[a.nr_atendimento] ?? (a.auxiliar_de_trafego ?? '')}
                                        onChange={(e) =>
                                            setAuxiliar(prev => ({ ...prev, [a.nr_atendimento]: e.target.value }))
                                        }
                                ...
                                <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                    ...
                                    <input
                                        type="text"
                                        value={fiscal[a.nr_atendimento] ?? (a.fiscal ?? '')}
                                        onChange={(e) =>
                                            setFiscal(prev => ({ ...prev, [a.nr_atendimento]: e.target.value }))
                                        }
                                ...
                                <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                                    ...
                                    <input
                                        type="time"
                                        step={60}
                                        value={chegada[a.nr_atendimento] ?? (a.chegada_na_garagem?.slice(0, 5) ?? '')}
                                        onChange={(e) =>
                                            setChegada(prev => ({ ...prev, [a.nr_atendimento]: e.target.value }))
                                        }
                                    />
                                </label>
